...
namespace {
	...
	void FlutterTtsPlugin::RegisterWithRegistrar(
		...
		try {
			...
			// Some Windows editions do not provide speech or media components.
			// Never let a WinRT activation failure cross the plugin C ABI and
			// terminate the whole application during startup.
			return;
		}
		methodChannel =
...
namespace {
	...
	void FlutterTtsPlugin::RegisterWithRegistrar(
		...
		try {
			...
			// The SAPI constructor can fail on stripped-down Windows images.
			// Treat TTS as unavailable instead of terminating the application.
			return;
		}
		methodChannel =
			std::make_unique<flutter::MethodChannel<flutter::EncodableValue>>(
