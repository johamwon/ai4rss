...
> 2026-08-06：INTEL-006 已完成。播客问答先在可信转录中筛选证据，无匹配时零 Provider 调用；回答陈述必须引用本次证据并由 River 物化精确时间轴、说话人和原文。每日音频简报默认单人播报，可选严格交替的 host/guest 对谈，每个 Turn 必须引用可信来源；源内容和生成脚本分别经过安全门禁。脚本/音频按剩余预算分阶段精确计费，取消后的晚返回成本仍幂等记录。固定 Replay 覆盖问答证据、拒答、伪造引用、对谈、安全拦截和取消计费。

> 2026-08-06：INTEL-007 已完成。FreshRSS 采用官方 Google Reader API，Miniflux 采用官方 `/v1` API；账户仅接受无凭据 HTTPS 基址，Token 始终脱敏。拉取契约统一输出完整订阅、有界状态对账集合和单调游标，状态写回有界；FreshRSS 重查有限窗口以捕获旧条目的状态变化，Miniflux 使用 `changed_after` 增量。相同规范 URL 在多账户间复用一个本地来源，同时保留账户级远端映射和阅读/收藏状态；旧状态不能覆盖新状态，移除账户只在最后一个映射消失时清理本地来源。固定 Replay 覆盖两种适配器、重复源、游标、状态同步和账户移除。
...
> 2026-08-10：READ-006 完成中文字体体系与本地自定义字体闭环；设计系统统一三端常见中文字体回退，设置页支持显式导入、预览和恢复默认，字体文件通过格式、大小与 SHA-256 校验后仅保存在设备应用目录。
...
> 2026-08-09：REL-001/005/006/007 可自动化核心已完成。Fast Lane 增加候选发布审计；Release 保存结构化准备报告和依赖清单，并继续只生成带 SHA-256 的 Draft Release。商店严格门禁会对模板应用标识、Debug Signing、缺失的真机/账号/签名/法务证据失败关闭；延期支付不阻塞永久免费产品发布，但付费入口保持关闭。证据矩阵见 `docs/RELEASE_READINESS.md`。

> 2026-08-09：BYOK 云能力第一阶段采用客户端直连。AI、TTS 与播客转录共用一个设置入口和无凭据日志的连接检查；Key 分能力保存在三端系统安全存储。首期落地 OpenAI-compatible AI、`audio/speech` 与 `audio/transcriptions` 适配器，River 不托管用户 Key。大于 100 MiB 的播客、持久云 TTS 缓存和长任务恢复继续使用既有云端口，待实际云资源阶段实现。
