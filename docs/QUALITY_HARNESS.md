...
行为隐私设置必须使用真实 SQLite 覆盖默认值、重启持久化和“关闭后零新增”。保留期测试必须覆盖边界时间；清空测试必须同时验证事件表为空、`secure_delete` 开启和 WAL 截断后的文件不残留测试事件标识。导出只允许包含版本化行为信封与设置，禁止通过文章关联带入标题、URL、正文、笔记或 AI 输出。设置表迁移必须保留 v14 Fixture，并覆盖建表后中断恢复和禁用选择不丢失。
