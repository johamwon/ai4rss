...
4. 阶段 13 COM-002：实现能力级 UsageGrant/UsageLedger、80/100% 提醒和失败返还。
...
- `river_commerce`：14 个测试通过，覆盖永久 Free 矩阵、访客访问、选择性 Pro 授权、规范载荷与脱敏、账户绑定、伪造、回滚/突变、未来/过期、离线缓存、在线刷新、试用降级、损坏缓存和 Free 提权拒绝；原有幂等用量测试继续通过。
- `river_test_harness`：15 组共 87 项检查通过，新增访客 Free、已验签 Pro、有界离线缓存、试用过期、伪造快照和 UI 商品标识零引用 Replay；继续覆盖云治理、播客转录、云 TTS、云端全文 SSRF、托管 AI 故障/超时/重复/降级、本地排序实验、生产排序、Fixture、Feed、正文提取、AI 黄金集与供应商/长文/缓存 Replay。
...
- Harness：fixtures 23/23、feeds 3/3、extraction 7/7、cloud extraction replay 5/5（公网地址固定、私网字面量零 Transport 调用）、cloud TTS replay 5/5（5 次 Provider 调用、17,500 毫秒计费时长、取消已传播、诊断无私人内容）、podcast transcription replay 5/5（5 次摄取、4 次转录、恢复跳过已完成阶段、隐私删除完整）、cloud governance replay 4/4（成本熔断、选择性关闭、伪造拒绝、本地阅读 1 次）、commerce entitlement replay 6/6（Free 检查 18 次、Pro 检查 3 次、伪造拒绝 1 次、UI 商品标识引用 0）、AI replay 8/8（必要事实 24/24、禁用声明命中 0/16、中英双语、八类内容、四个高风险样例）、AI provider replay 5/5、AI long replay 1/1、AI cache replay 1/1、managed AI gateway replay 4/4（7 次有界调用、750 微单位已接受成本、诊断无私人内容）、ranking 7/7、ranking experiment replay 3/3。
