...
1. READ-007：键盘、焦点、屏幕阅读器、对比度和大字体可访问性。
2. TTS-001：文章分段、队列与播放状态机。
...
- `river_data`：42 个测试通过，新增飞行模式持久入队、联网恢复、有限自动重试、终态手动重试和冷启动缓存读取，并继续覆盖 v1/v2/v3→v4、中断恢复及 10,000 篇文章 P95 0.7ms 基准。
- `river_app`：42 个测试通过，新增离线下载状态与失败重试覆盖，并继续覆盖全文恢复、离线订阅、后台刷新、渐进阅读和五组跨尺寸/主题 Golden。
