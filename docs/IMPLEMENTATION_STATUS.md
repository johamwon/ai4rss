...
- DATA-001：Drift/SQLite v17 本地数据库及 FTS5 索引、外键、唯一键和级联规则；v2～v5 依次增加 Feed 正文、阅读设置、全文索引和音频断点，v6～v7 增加同步副本/冲突历史，v8 分离 Podcast 节目、分集和下载状态，v9 将下载文件绑定到来源 URL，v10 增加文章与 Podcast 共用的持久播放队列，v11 保存 Podcasting 2.0 章节与文字稿引用，v12 增加文章高亮与笔记锚点，v13 增加统一知识来源与外部映射，v14 增加已校验 AI 产物缓存，v15 增加本地行为采集与保留设置，v16 增加来源/主题调整和屏蔽覆盖层，v17 增加自动摘要策略与按本地日原子预留的用量账本，并保留旧版本升级兼容。
- DATA-003：v0、v1、v2、v3、v4、v9、v10、v11、v12、v13、v14、v15 与 v16 Fixture 及 N-2 升级演练已完成；覆盖全部前代 Fixture→v17、各阶段迁移中断后的幂等恢复、v14→v15、v15→v16、v16→v17、偏好禁用选择保留、v16 覆盖层保留、v17 自动摘要策略建表中断恢复及当前 v17 打开。
...
4. 阶段 11 RANK-006：实现推荐质量、摘要命中率、AI 延迟与有界成本的本地可解释度量及调优闭环。
...
- `river_domain`：31 个测试通过，新增默认关闭、有界日限额、高匹配阈值、网络状态和原子用量预留契约；继续覆盖有界来源/主题偏好控制、七类阅读事件 v1 wire 往返、未来版本拒绝、事件专属载荷边界、稳定幂等键、AI 产物身份、连接器、知识对象、来源引用、DOM/文本双锚点、跨来源队列、音频、后台刷新与核心模型。
- `river_data`：121 个测试通过，新增 Drift v17 自动摘要策略、并发安全日额度账本、跨日未完成预留迁移、无重试消耗任务延后、v16→v17 迁移和中断恢复；继续覆盖偏好控制、阅读事件顺序/并发幂等、AI 产物、知识导出耐久队列、来源并发去重、外部映射、全部历史迁移、高亮/笔记、Podcasting 2.0、统一队列、网络/磁盘/租约恢复、同步与 10,000 篇文章 P95 <500ms 自动门槛。
...
- `river_test_harness`：8 组共 55 项检查通过，新增生产排序 Replay 的权重顺序、半衰期、点击封顶、模型版本和来源/主题画像断言及错误版本负向门禁；继续覆盖 Fixture、Feed、正文提取、AI 黄金集、供应商/长文/缓存 Replay 和确定性时间/HTTP/AI 场景。
- `river_preferences`：30 个测试通过，新增真实排序贡献解释、探索理由与用户偏好覆盖；继续覆盖主来源封顶、探索配额、强负反馈、主题屏蔽、固定候选多因子解释、1,000 候选/信号属性、画像、重复点击封顶、阅读状态机及模型版本。
- `river_app`：102 个测试通过，新增高匹配自动摘要、Wi-Fi/日额度门禁、失败释放、缓存免计费、全文提取、崩溃恢复、关闭取消和后台刷新恢复纵向覆盖；继续覆盖智能排序、精确“为什么推荐”、行为采集隐私、AI 摘要确认/恢复、文章→高亮/笔记→知识保存、响应式知识库、Markdown/Notion、同步、全局迷你播放器、收听队列、播客、阅读器、后台媒体、Windows SMTC 与跨尺寸 Golden。
...
- Harness：fixtures 23/23、feeds 3/3、extraction 7/7、AI replay 8/8（必要事实 24/24、禁用声明命中 0/16、中英双语、八类内容、四个高风险样例）、AI provider replay 5/5、AI long replay 1/1、AI cache replay 1/1、ranking 7/7。
