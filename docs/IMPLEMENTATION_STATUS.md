...
- READ-007：首页在 Windows/平板宽屏使用左侧订阅导航＋全高文章流，窄屏默认折叠订阅管理，使文章列表获得主要视口。阅读器从已净化原文 HTML 恢复标题、强调、引用、代码、链接、列表、图注和表格语义样式，同时保持规范纯文本偏移不变，既保留原文层级又兼容现有选区、高亮、笔记、TTS 当前句和阅读进度。
...
- AI-007：OpenAI-compatible 响应兼容层可识别 JSON 围栏、前置说明、受限单层包装、content block、parsed object 与缺失 usage，同时仍以原严格 Schema 拒绝缺字段、错语言、额外字段和非法值；阅读页分别说明格式、完整性、语言和字段校验错误并允许恢复。文章流支持显式选择 2～20 篇生成综合摘要，调用前披露 Provider、模型、篇数、字符数和最多两次请求；`river.multi-article-summary.v1` 输出总览、共同主题、逐篇要点及关联差异，强制每个本地文章 ID 恰好出现一次，总输入公平限制在 60,000 字符。
...
- BYOK-002：云 TTS 新增 Fish Audio 专用预设和适配器，固定官方 API 域名，支持 `s2.1-pro`、`s2.1-pro-free`、`s2-pro`、`s1`、默认音色及 Voice Model ID。合成按官方协议使用 `/v1/tts`、Bearer Key、`model` 请求头和可选 `reference_id`；类型化高级配置覆盖 temperature、top_p、音量、分块、规范化、延迟、格式专用码率和 quality-guard，语速按官方 0.5～2.0 限制，避免 River 2.5/3.0 倍设置触发参数错误。连接检查使用只读额度接口，不生成计费语音；旧安全仓库配置无参数对象时应用官方默认值。
