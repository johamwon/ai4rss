...
2. 阶段 6 SYNC-007：实现密文同步服务、配额、限流、备份、监控和恢复演练。
...
- `river_sync`：68 个测试通过，新增账号体验状态机、失败恢复、安全退出与同账号云删除回执门槛；继续覆盖字段/语义合并、幂等重复、墓碑压缩、六类载荷、双设备分页、72 次随机离线收敛、AES/X25519/HKDF、恢复和设备生命周期。
