...
4. 阶段 12 CLOUD-004：实现播客上传/拉取转录、章节和摘要任务。
...
- `river_ai`：58 个测试通过，新增可信套餐/能力路由、调用方模型隔离、Provider 密钥代理边界、账户限流、总请求/单路由超时、熔断半开恢复、质量回退、精确成本、并发幂等合并、冲突拒绝、有界保留和诊断隐私；继续覆盖短文/长文缓存、段落分块、来源引用、费用预检、五类 BYOK、严格摘要 Schema、一次修复和 Prompt 版本不可变。
- `river_test_harness`：12 组共 72 项检查通过，新增云 TTS 精确计费、并发重复、取消晚返回、正文 revision 变化和计费网络拒绝 Replay；继续覆盖云端全文 SSRF、托管 AI 故障/超时/重复/降级、本地排序实验、生产排序、Fixture、Feed、正文提取、AI 黄金集与供应商/长文/缓存 Replay。
