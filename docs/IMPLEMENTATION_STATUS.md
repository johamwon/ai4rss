...
1. READ-002：渐进阅读页，先显示 Feed/缓存内容，再无闪烁替换完整正文。
2. READ-003/004：阅读排版设置和已读、收藏、稍后读、进度等持久状态操作。
...
- `river_feed`：24 个测试通过，新增文章视图查询契约和中英文阅读时长估算。
- `river_data`：19 个测试通过，新增收件箱、未读、收藏、稍后读、文件夹与时间排序联表查询。
- `river_app`：12 个测试通过，覆盖刷新进度、文章筛选排序、1000 条惰性列表、滚动恢复、语义状态和安全重试。
- `river_extract`：26 个测试通过，覆盖缓存命中、请求合并、全局/同源并发、版本/验证器失效、哈希、旧缓存回退和动态渲染降级，以及安全清理、Feed 全文、微信和 Readability。
...
- 三端构建：最近一次 Android/iOS/Windows 主分支 Debug 构建及产物上传均已通过；READ-001 不修改原生平台代码或数据库 schema。
