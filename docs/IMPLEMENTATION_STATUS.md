...
- DATA-001：Drift/SQLite v15 本地数据库及 FTS5 索引、外键、唯一键和级联规则；v2～v5 依次增加 Feed 正文、阅读设置、全文索引和音频断点，v6～v7 增加同步副本/冲突历史，v8 分离 Podcast 节目、分集和下载状态，v9 将下载文件绑定到来源 URL，v10 增加文章与 Podcast 共用的持久播放队列，v11 保存 Podcasting 2.0 章节与文字稿引用，v12 增加文章高亮与笔记锚点，v13 增加统一知识来源与外部映射，v14 增加已校验 AI 产物缓存，v15 增加本地行为采集与保留设置，并保留旧版本升级兼容。
- DATA-003：v0、v1、v2、v3、v4、v9、v10、v11、v12、v13 与 v14 Fixture 及 N-2 升级演练已完成；覆盖全部前代 Fixture→v15、各阶段迁移中断后的幂等恢复、v13→v14、v14→v15、v15 建表中断后禁用选择保留及当前 v15 打开。
...
4. 阶段 11 RANK-004：实现“为什么推荐”、查看/编辑/关闭/清空画像。
...
- `river_domain`：28 个测试通过，新增本地行为设置边界和无正文、稳定排序的版本化导出；继续覆盖七类阅读事件 v1 wire 往返、未来版本拒绝、事件专属载荷边界、稳定幂等键、AI 产物身份、连接器、知识对象、来源引用、DOM/文本双锚点、跨来源队列、音频、后台刷新与核心模型。
- `river_data`：111 个测试通过，新增未确认零采集、持久禁用、保留边界、无正文导出、文件级安全清空、v14→v15 及中断恢复；继续覆盖阅读事件顺序/并发幂等、AI 产物、知识导出耐久队列、来源并发去重、外部映射、全部历史迁移、高亮/笔记、Podcasting 2.0、统一队列、网络/磁盘/租约恢复、同步与 10,000 篇文章 P95 <500ms 自动门槛。
...
- `river_preferences`：27 个测试通过，新增主来源封顶、探索配额、强负反馈、主题屏蔽、乱序重放、来源不足、定制阈值和恶意输入覆盖；继续验证固定候选多因子解释、1,000 候选/信号属性、画像、重复点击封顶、阅读状态机及模型版本。
- `river_app`：87 个测试通过，新增首启本机采集选择、可访问隐私说明、持久开关、保留期、复制导出和安全清空；继续覆盖 AI 摘要确认/恢复、文章→高亮/笔记→知识保存、响应式知识库、Markdown/Notion、同步、全局迷你播放器、收听队列、播客、阅读器、后台媒体、Windows SMTC 与跨尺寸 Golden。
