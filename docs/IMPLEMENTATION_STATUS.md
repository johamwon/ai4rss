...
1. EXT-008：全文失败恢复 UX 与原文/重试入口。
2. READ-006/007：离线下载、网络恢复、失败任务重试及键盘快捷键。
3. TTS-001：文章分段、队列与播放状态机。
...
- `river_app`：38 个测试通过，新增离线地址保留、恢复后重试、多 Feed 选择和错误脱敏覆盖，并继续覆盖后台刷新、渐进阅读和五组跨尺寸/主题 Golden。
- `river_extract`：29 个测试通过，新增完整 Feed 零网页请求、摘要静态下载和失败后平台回退编排覆盖。
- `river_platform`：15 个测试通过，新增链路状态映射、状态流和适配器失败降级覆盖，并继续覆盖三端后台调度及动态渲染契约。
