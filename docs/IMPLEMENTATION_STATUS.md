...
2. 阶段 6 SYNC-003：实现客户端加密、密钥恢复提示与安全存储。
...
- `river_sync`：28 个测试通过，新增首设备无密码登录、令牌/邮箱脱敏、重复完成幂等、挑战和会话过期、离线保留、待审批设备阻断、包装密钥接收方校验、审批激活、撤销传播、轮换要求、最后设备保护和退出仅清会话；并继续覆盖版本向量、密文信封、批次游标、墓碑和对象冲突规则。
