...
4. 阶段 8 KB-005：实现单篇/批量 Markdown 文件导出和远程/本地图片策略。
...
- `river_knowledge`：6 个测试通过，规范 Markdown 覆盖完整结构、Front Matter 注入防护、换行规范化、集合字节稳定排序、非法字符/长度和同名冲突；导出服务只向耐久管理器提交目标，规范 SHA-256 对集合顺序稳定且能检测笔记内容变化。
