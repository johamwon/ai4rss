...
2. 阶段 6 SYNC-004：实现订阅、文件夹、状态、设置、进度和知识元数据增量同步。
...
- `river_sync`：37 个测试通过，新增跨运行时 AES-256-GCM 固定向量、密文无明文、AAD 篡改、错误/销毁密钥、X25519+HKDF 接收方隔离、待审批设备包裹与撤销设备拒绝、密钥恢复、轮换和恢复错误，并继续覆盖认证设备生命周期、版本向量、批次游标、墓碑和冲突规则。
