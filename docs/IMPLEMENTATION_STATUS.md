...
2. 阶段 6 SYNC-005：实现字段级/语义冲突合并、去重、墓碑压缩和幂等重放。
...
- `river_data`：51 个测试通过，新增同步副本/outbox/cursor/冲突跨重启、作用域确认、游标事务回滚、v1→v6 和中断 v6 幂等迁移，并继续覆盖持久任务、旧版本升级、文章与播客断点及 10,000 篇文章 P95 <500ms 自动门槛。
...
- `river_sync`：53 个测试通过，新增六类载荷/wire codec、双设备分页增量往返、同步前设备授权、离线 outbox、墓碑、部分确认、非前进游标、错误载荷、事务失败、并发暂存和 72 次随机离线收敛，并继续覆盖 AES/X25519/HKDF、恢复、认证设备生命周期、版本向量和冲突分类。
