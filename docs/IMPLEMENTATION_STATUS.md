...
- DATA-001：Drift/SQLite v10 本地数据库及 FTS5 索引、外键、唯一键和级联规则；v2～v5 依次增加 Feed 正文、阅读设置、全文索引和音频断点，v6～v7 增加同步副本/冲突历史，v8 分离 Podcast 节目、分集和下载状态，v9 将下载文件绑定到来源 URL，v10 增加文章与 Podcast 共用的持久播放队列，并保留旧版本升级兼容。
- DATA-003：v0、v1、v2、v3、v4 与 v9 Fixture 及 N-2 升级演练已完成；覆盖 v1/v2/v3/v4→v10、各阶段迁移中断后的幂等恢复、v8→v9、v9→v10 及当前 v10 打开。
...
3. 阶段 7 POD-005 真机验收：完成 Android/iOS/Windows 后台连续播放、锁屏切换队列项和页面往返矩阵。

## 最近验证

- Fast Lane：通过，静态分析 0 问题。
- `river_feed`：36 个测试通过，新增 Podcast 仅最新、所有新分集和切换策略下载历史的策略覆盖；继续覆盖 Podcast RSS 节目/分集模型、enclosure 安全筛选、iTunes 元数据、稳定 GUID、条件刷新、搜索、RSS/Atom/JSON Feed、发现、文章刷新、OPML 与 HTTP 边界。
- `river_domain`：12 个测试通过，新增跨来源队列当前项和文章正文修订约束，并继续覆盖统一音频快照的正文/播客位置约束、来源载荷、正文版本/分段索引、播放参数边界、后台刷新策略与核心模型。
- `river_data`：75 个测试通过，新增统一队列混排、去重、移动、消费、文件数据库重启、v9 旧库和 v10 中断恢复；继续覆盖 Podcast 网络切换、磁盘满、损坏媒体、冷启动租约恢复、删除重下、节目/分集事务、v1→v10 幂等迁移、同步与持久任务及 10,000 篇文章 P95 <500ms 自动门槛。
- `river_app`：65 个测试通过，新增全局迷你播放器、完整播放页、混排前后项、播放暂停、倍速、定时停止、应用级路由、320 像素窄屏及播放页 Golden；继续覆盖收听队列、播客、本地文件优先播放、同步状态、冲突历史、阅读器、后台媒体、Windows SMTC 与五组阅读器跨尺寸 Golden。
- `river_design_system`：2 个测试通过，浅色与深色高对比主题的关键文字组合均达到 WCAG AA 4.5:1。
- `river_audio`：33 个测试通过，新增统一持久队列跨来源选择、完成后连续消费、节目倍速和解析失败保留恢复；继续覆盖文章/Podcast 类型路由、两小时长文预算、有界预取、焦点拒绝、系统媒体命令、中断恢复、进度写入、定时暂停与跨语言分句。
...
- `river_platform`：40 个测试通过，新增真实 HTTP Range/If-Range 续传、服务端忽略 Range 回退、损坏音频拒绝和凭据重定向拒绝；继续覆盖 Podcast 远程/本地媒体播放、安全仓库、系统音频会话、系统 TTS、外部原文、链路状态、后台调度及动态渲染契约。
