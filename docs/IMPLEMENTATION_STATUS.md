...
- PREF-003：行为事件默认仅写本地，v15 持久化采集开关与 1～3650 天保留期，默认开启/90 天；关闭后仓库事务性返回 captureDisabled 且不新增记录。保留清理严格保留边界事件，完整清空启用 SQLite secure_delete 并截断 WAL。`river.reading-event-export` v1 只导出设置和按时间/ID 排序的行为信封，不关联标题、URL、正文、笔记或 AI 输出；v14→v15 及建表中断恢复均保留禁用选择。
...
4. 阶段 10 PREF-004：增加用户可读的行为说明和首次启用提示。
...
- `river_data`：111 个测试通过，新增默认本地采集、持久禁用、保留边界、无正文导出、文件级安全清空、v14→v15 及中断恢复；继续覆盖阅读事件顺序/并发幂等、AI 产物、知识导出耐久队列、来源并发去重、外部映射、全部历史迁移、高亮/笔记、Podcasting 2.0、统一队列、网络/磁盘/租约恢复、同步与 10,000 篇文章 P95 <500ms 自动门槛。
...
- `river_preferences`：11 个测试通过，新增前台、可见分屏、后台、锁屏、页面不可见、挂机超时、跳读、增量 flush、倒退时钟及长会话切分覆盖；继续验证负反馈权重高于点击弱信号。
