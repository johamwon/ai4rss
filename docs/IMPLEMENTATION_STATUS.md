...
2. 阶段 6 SYNC-006：实现同步状态、冲突历史、重试、退出账号和云数据删除 UI。
...
- `river_data`：54 个测试通过，新增 seen-mutation、防碰撞、合并决议审计与加密派生 outbox 原子事务，并覆盖 v1→v7、中断 v6 和中断 v7 幂等迁移；继续覆盖持久任务、文章与播客断点及 10,000 篇文章 P95 <500ms 自动门槛。
- `river_app`：53 个测试通过，新增 Android 安全仓库 API/备份边界和恢复码警告、确认门槛、可选择只读展示，并继续覆盖 Android 后台媒体注册、iOS 音频后台模式、Windows SMTC 构建契约、阅读器控制链路与五组跨尺寸 Golden。
...
- `river_sync`：65 个测试通过，新增字段版本兼容、确定性字段/语义合并、音频最远进度、派生 mutation 收敛、幂等重复、mutation 碰撞、墓碑优先级和 30 天/活跃游标压缩门槛；继续覆盖六类载荷、双设备分页、离线 outbox、72 次随机离线收敛、AES/X25519/HKDF、恢复和设备生命周期。
