...
4. 阶段 8 KB-004：实现连接器 Create/Update/Delete/Status 协议和幂等任务队列。
...
- `river_domain`：19 个测试通过，新增统一知识对象、来源引用、外部映射与边界校验；继续覆盖 DOM/文本双锚点、跨来源队列、统一音频快照、播放参数、后台刷新策略与核心模型。
- `river_data`：90 个测试通过，新增知识来源并发去重、首次身份/保存时间保留、知识与映射的过期写保护、外部映射幂等更新/级联、原文删除后知识保留、损坏行关闭、历史重复项无损迁移、v12 旧库与 v13 中断恢复；继续覆盖高亮/笔记、Podcasting 2.0 元数据、统一队列、网络/磁盘/租约恢复、v1→v13 幂等迁移、同步与持久任务及 10,000 篇文章 P95 <500ms 自动门槛。
- `river_knowledge`：6 个测试通过，新增规范 Markdown 的完整结构、Front Matter 注入防护、换行规范化、集合字节稳定排序、非法字符/长度和同名冲突覆盖；继续验证连接器幂等键基于 KnowledgeItem，规范 SHA-256 对集合顺序稳定且能检测笔记内容变化。
