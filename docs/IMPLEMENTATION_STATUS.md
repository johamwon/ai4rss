...
- DATA-001：Drift/SQLite v16 本地数据库及 FTS5 索引、外键、唯一键和级联规则；v2～v5 依次增加 Feed 正文、阅读设置、全文索引和音频断点，v6～v7 增加同步副本/冲突历史，v8 分离 Podcast 节目、分集和下载状态，v9 将下载文件绑定到来源 URL，v10 增加文章与 Podcast 共用的持久播放队列，v11 保存 Podcasting 2.0 章节与文字稿引用，v12 增加文章高亮与笔记锚点，v13 增加统一知识来源与外部映射，v14 增加已校验 AI 产物缓存，v15 增加本地行为采集与保留设置，v16 增加来源/主题调整和屏蔽覆盖层，并保留旧版本升级兼容。
- DATA-003：v0、v1、v2、v3、v4、v9、v10、v11、v12、v13、v14 与 v15 Fixture 及 N-2 升级演练已完成；覆盖全部前代 Fixture→v16、各阶段迁移中断后的幂等恢复、v14→v15、v15→v16、v15 建表中断后禁用选择保留、v16 部分列中断后覆盖层保留及当前 v16 打开。
...
4. 阶段 11 RANK-005：实现仅对高匹配文章自动摘要及 Wi-Fi/日限额。
...
- `river_domain`：29 个测试通过，新增有界来源/主题偏好控制、稳定比较与无正文版本化导出；继续覆盖七类阅读事件 v1 wire 往返、未来版本拒绝、事件专属载荷边界、稳定幂等键、AI 产物身份、连接器、知识对象、来源引用、DOM/文本双锚点、跨来源队列、音频、后台刷新与核心模型。
- `river_data`：114 个测试通过，新增 Drift v16 偏好控制持久化、v15→v16 迁移、中断恢复与原子画像清空；继续覆盖阅读事件顺序/并发幂等、AI 产物、知识导出耐久队列、来源并发去重、外部映射、全部历史迁移、高亮/笔记、Podcasting 2.0、统一队列、网络/磁盘/租约恢复、同步与 10,000 篇文章 P95 <500ms 自动门槛。
...
- `river_test_harness`：6 个包级测试通过，新增生产排序 Replay 的权重顺序、半衰期、点击封顶、模型版本和来源/主题画像断言及错误版本负向门禁；继续覆盖 AI 黄金集、Fake Connector 和确定性时间/HTTP/AI 场景。
- `river_preferences`：30 个测试通过，新增真实排序贡献解释、探索理由与用户偏好覆盖；继续覆盖主来源封顶、探索配额、强负反馈、主题屏蔽、固定候选多因子解释、1,000 候选/信号属性、画像、重复点击封顶、阅读状态机及模型版本。
- `river_app`：92 个测试通过，新增智能排序、精确“为什么推荐”、画像查看/编辑/关闭、来源屏蔽和原子清空纵向覆盖；继续覆盖行为采集隐私、AI 摘要确认/恢复、文章→高亮/笔记→知识保存、响应式知识库、Markdown/Notion、同步、全局迷你播放器、收听队列、播客、阅读器、后台媒体、Windows SMTC 与跨尺寸 Golden。
