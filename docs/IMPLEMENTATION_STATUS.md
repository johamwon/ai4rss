...
- INTEL-004：复用现有 Notion 内容哈希与外部映射实现增量跳过/更新，新增供应商无关 `KnowledgeDocumentStore` 和 WebDAV Transport 端口。Obsidian 连接器将规范 Markdown 原子写入受限目录，重复创建按字节幂等，标题变化保留稳定路径，revision 冲突显式且可重试；路径穿越失败关闭。WebDAV 默认仅 HTTPS，创建使用 `If-None-Match: *`，更新和删除使用 ETag `If-Match`，并将认证、冲突、限流、容量、离线和超时映射为稳定错误。诊断不含正文、凭据、响应体或完整目标 URI；五类固定 Replay 全部通过。
- INTEL-005：完成 Android/iOS/Windows 共用的 IMA 用户辅助互操作。单条知识生成规范 UTF-8 Markdown，多条或资源包生成确定性 ZIP；系统分享携带单个内存文件及安全文件名，文件保存使用平台选择器，“打开 ima”只允许固定 `https://ima.qq.com/` 公开入口。包大小上限 200 MiB，取消与不可用分离，平台异常安全降级，本地知识不受影响。实现显式声明不使用原生私有 API，并拒绝私有 Scheme、凭据、查询、片段、非官方主机和私有路径；诊断只含操作、结果、条目数和字节数。五类固定 Replay 全部通过。
- INTEL-006：完成供应商无关的播客转录问答与每日音频简报编排。问答先做确定性转录证据筛选，无证据时零 Provider 调用；每条陈述必须引用本次发送的 1～5 个 Segment，最终引用由 River 物化精确起止时间、说话人和原文，伪造/重复/缺失引用失败关闭。每日简报绑定 UTC 日期、语言、样式、来源哈希和总成本预算，默认 narration，可选严格交替 host/guest 对谈；每个 Turn 必须引用可信来源。源内容和脚本双安全门禁，脚本/音频按剩余预算分阶段计费，非法输出和取消后的晚返回仍记录已发生成本；成功重试不重复调用。六类固定 Replay 全部通过。
...
4. 阶段 14 INTEL-007：实现 FreshRSS、Miniflux 账户适配及多账户来源映射。
...
- `river_test_harness`：23 组共 142 项检查通过，新增播客音频智能 6 项 Replay：可信问答、零 Provider 拒答、伪造引用拒绝、对谈简报、安全拦截和取消晚返回计费；继续覆盖 IMA、便携连接器、知识问答、知识搜索黄金集、向量生命周期、永久 Free、商业/云治理、音频、排序、提取及 AI Replay。
...
- Harness：fixtures 23/23、feeds 3/3、extraction 7/7、cloud extraction replay 5/5、cloud TTS replay 5/5、podcast transcription replay 5/5、podcast audio intelligence replay 6/6（可信问答/零 Provider/伪造引用/对谈/安全/取消计费各 1，诊断私有内容 0）、cloud governance replay 4/4、commerce entitlement replay 6/6、usage ledger replay 5/5、free product replay 18/18、knowledge vector replay 5/5、knowledge search replay 6/6（Recall@K/Precision@K 1.00、证据 10/10）、knowledge question replay 5/5（回答 1、零 Provider 拒答 1、Provider 拒答 1、非法输出拒绝 2、诊断私有内容 0）、portable connector replay 5/5（Obsidian 幂等/冲突、WebDAV 条件写入/限流/离线，诊断私有内容 0）、IMA portable replay 5/5（Markdown/ZIP/取消/公开入口/私有 URI 拒绝，私有 API false，诊断私有内容 0）、AI replay 8/8、AI provider replay 5/5、AI long replay 1/1、AI cache replay 1/1、managed AI gateway replay 4/4、ranking 7/7、ranking experiment replay 3/3。
