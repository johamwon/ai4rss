...
4. 阶段 12 CLOUD-005：建立成本追踪、模型/能力级 Span 和远程 Kill Switch。
...
- `river_ai`：67 个测试通过，新增播客远程源预检、六小时转录、格式/声明复验、中断检查点恢复、并发/完成幂等、取消、冲突、严格章节摘要和隐私删除；继续覆盖可信托管路由、限流/超时/熔断/质量回退/精确成本、短文/长文缓存、段落分块、来源引用、费用预检、五类 BYOK、严格摘要 Schema、一次修复和 Prompt 版本不可变。
- `river_test_harness`：13 组共 77 项检查通过，新增播客六小时音频、格式错误、中断恢复、并发重复和隐私删除 Replay；继续覆盖云 TTS、云端全文 SSRF、托管 AI 故障/超时/重复/降级、本地排序实验、生产排序、Fixture、Feed、正文提取、AI 黄金集与供应商/长文/缓存 Replay。
