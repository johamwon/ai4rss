...
- DATA-001：Drift/SQLite v11 本地数据库及 FTS5 索引、外键、唯一键和级联规则；v2～v5 依次增加 Feed 正文、阅读设置、全文索引和音频断点，v6～v7 增加同步副本/冲突历史，v8 分离 Podcast 节目、分集和下载状态，v9 将下载文件绑定到来源 URL，v10 增加文章与 Podcast 共用的持久播放队列，v11 保存 Podcasting 2.0 章节与文字稿引用，并保留旧版本升级兼容。
- DATA-003：v0、v1、v2、v3、v4、v9 与 v10 Fixture 及 N-2 升级演练已完成；覆盖 v1/v2/v3/v4→v11、各阶段迁移中断后的幂等恢复、v8→v9、v9→v10、v10→v11 及当前 v11 打开。
...
- `river_data`：77 个测试通过，新增 v10 Podcast 元数据旧库与 v11 中断恢复，并覆盖章节/文字稿引用往返；继续覆盖统一队列、Podcast 网络切换、磁盘满、损坏媒体、冷启动租约恢复、删除重下、节目/分集事务、v1→v11 幂等迁移、同步与持久任务及 10,000 篇文章 P95 <500ms 自动门槛。
