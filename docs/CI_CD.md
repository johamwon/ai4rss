...
- Windows 托管 Runner 会真实创建、查询并删除当前用户的 River 后台刷新计划任务；测试使用唯一固定任务名并在 `finally` 中清理。
- Nightly 当前只执行已落地的离线语料和平台构建；Live Canary、Android/iOS 真机矩阵、AI Live Eval 与性能基线将在对应 Harness 能力实现后接入。
