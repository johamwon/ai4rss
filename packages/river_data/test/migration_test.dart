...
void main() {
  test('v1 fixture migrates to v18 without losing article state', () async {
    ...
    expect(await _userVersion(migrated), 18);
    expect(
  ...
  test('interrupted v2 column addition retries idempotently', () async {
    ...
    expect(await _userVersion(recovered), 18);
  });
  ...
  test('v3 fixture creates a searchable index without data loss', () async {
    ...
    expect(await _userVersion(current), 18);
  });
  ...
  test(
    'v14 fixture adds default local behavior settings without data loss',
    () async {
      final fixture = await _materializeFixture(
        'v014_reading_behavior_settings.sql',
      );
      final migrated = await _openFixture(fixture);

      final marker = await migrated
          .customSelect("SELECT value FROM migration_marker WHERE id = 'v14'")
          .getSingle();
      expect(
        marker.read<String>('value'),
        'preserve-before-reading-behavior-settings',
      );
      expect(
        await _columnNames(migrated, 'reading_behavior_settings_rows'),
        containsAll(<String>[
          'id',
          'capture_enabled',
          'retention_days',
  ...
  test(
    'interrupted v15 settings creation preserves a disabled preference',
    () async {
      final fixture = await _materializeFixture(
        'v014_reading_behavior_settings.sql',
      );
      ...
      expect(
        await DriftReadingEventRepository(recovered).readSettings(),
        const ReadingBehaviorSettings(captureEnabled: false, retentionDays: 30),
      );
      expect(await _userVersion(recovered), 18);
    },
  );
  ...
  test(
    'v15 fixture adds bounded preference controls without data loss',
    () async {
      final fixture = await _materializeFixture('v015_preference_controls.sql');
      final migrated = await _openFixture(fixture);

      final marker = await migrated
          .customSelect("SELECT value FROM migration_marker WHERE id = 'v15'")
          .getSingle();
      expect(
        marker.read<String>('value'),
        'preserve-before-preference-controls',
      );
      expect(
        await _columnNames(migrated, 'reading_behavior_settings_rows'),
        containsAll(<String>[
  ...
  test(
    'interrupted v16 control migration preserves completed columns',
    () async {
      final fixture = await _materializeFixture('v015_preference_controls.sql');
      final raw = sqlite.sqlite3.open(fixture.path);
      raw
        ..execute('''
        ALTER TABLE reading_behavior_settings_rows
        ADD COLUMN source_score_adjustments_json TEXT NOT NULL DEFAULT '{}'
      ''')
        ..execute('''
        UPDATE reading_behavior_settings_rows
        SET source_score_adjustments_json = '{"feed-1":2.0}'
      ''')
        ..close();
      final recovered = await _openFixture(fixture);
      ...
      expect(
        ...
        const ReadingBehaviorSettings(
          ...
          retentionDays: 30,
          preferenceControls: ReadingPreferenceControls(
            sourceScoreAdjustments: <String, double>{'feed-1': 2},
          ),
        ),
      );
      expect(
        await _columnNames(recovered, 'reading_behavior_settings_rows'),
        containsAll(<String>[
          'source_score_adjustments_json',
          'topic_score_adjustments_json',
          'blocked_source_ids_json',
  ...
  test('v16 fixture adds automatic summary policy and usage tables', () async {
    final fixture = await _materializeFixture('v016_automatic_summaries.sql');
    final migrated = await _openFixture(fixture);

    final marker = await migrated
        .customSelect("SELECT value FROM migration_marker WHERE id = 'v16'")
        .getSingle();
    expect(marker.read<String>('value'), 'preserve-before-automatic-summaries');
    expect(
      await _columnNames(migrated, 'automatic_summary_settings_rows'),
      containsAll(<String>[
        'enabled',
        'wifi_only',
        'daily_limit',
    ...
    expect(
      await _columnNames(migrated, 'automatic_summary_usage_rows'),
      containsAll(<String>[
        'idempotency_key',
        'article_id',
        'day_key',
    ...
    expect(
      await DriftAutomaticSummaryRepository(migrated).readSettings(),
      const AutomaticSummarySettings(),
    );
    expect(await _userVersion(migrated), 18);
  });

  test('interrupted v17 migration preserves an enabled policy', () async {
    final fixture = await _materializeFixture('v016_automatic_summaries.sql');
    final raw = sqlite.sqlite3.open(fixture.path);
    raw
      ..execute('''
    ...
    expect(
      await DriftAutomaticSummaryRepository(recovered).readSettings(),
      const AutomaticSummarySettings(
        enabled: true,
        wifiOnly: false,
        dailyLimit: 5,
    ...
    expect(
      await _columnNames(recovered, 'automatic_summary_usage_rows'),
      isNotEmpty,
    );
    expect(await _userVersion(recovered), 18);
  });
  ...
  test(
    'v17 fixture adds aggregate-only local ranking experiment tables',
    () async {
      final fixture = await _materializeFixture('v017_ranking_experiment.sql');
      final migrated = await _openFixture(fixture);

      final marker = await migrated
          .customSelect("SELECT value FROM migration_marker WHERE id = 'v17'")
          .getSingle();
      expect(
        marker.read<String>('value'),
        'preserve-before-ranking-experiment',
      );
      expect(
        await _columnNames(migrated, 'ranking_experiment_daily_metric_rows'),
        containsAll(<String>[
          'experiment_id',
          'arm',
          'day_key',
  ...
  test(
    'interrupted v18 migration preserves a completed enrollment table',
    () async {
      final fixture = await _materializeFixture('v017_ranking_experiment.sql');
      final raw = sqlite.sqlite3.open(fixture.path);
      raw
        ..execute('''
        CREATE TABLE ranking_experiment_enrollment_rows (
          id INTEGER NOT NULL PRIMARY KEY DEFAULT 1,
      ...
      ''')
        ..execute('''
        INSERT INTO ranking_experiment_enrollment_rows VALUES (
          1, 'ranking-time-control/1', 'personalized',
          1785945600, 1785945600
        )
      ''')
        ..close();
      final recovered = await _openFixture(fixture);

      final enrollment = await DriftRankingExperimentRepository(
        recovered,
      ).readEnrollment();
      expect(enrollment?.arm, RankingExperimentArm.personalized);
      expect(
        await _columnNames(recovered, 'ranking_experiment_daily_metric_rows'),
        isNotEmpty,
      );
      expect(await _userVersion(recovered), 18);
    },
